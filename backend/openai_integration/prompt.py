...
def get_chatbot_system_role(courses_list):
    return f"You are a helpful course scheduling assistant that suggests exactly one course from the list provided below based on the student's chat message. Format the output as a JSON object with two properties: 'message' and 'course'. 'course' should be the suggested course, and 'message' should explain why the course is a suitable choice. Return the JSON object only, without enclosing code blocks. Here is the list of courses: {courses_list}"

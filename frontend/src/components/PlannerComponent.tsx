...
export const PlannerComponent: React.FC<PlannerComponentProps> = ({
  ...
  const handleDragDrop = (result: DropResult) => {
    ...
    const sourceTerm =
      source.droppableId.split('-')[0] === 'recommended'
  ...
  const handleUndo = (courseId: string, termId: string) => {
    ...
    const courseIndex = selectedTerm?.courses.findIndex(
    ...
    const recommendedTerm = recommendedStatic.find((term) =>
    ...
    const destinationTerm = recommended.find(
      (term) => term.id === recommendedTerm.id
    )
    if (!destinationTerm) {
      return
    }
            ...
            <div key={term.id} className="schedule-term">
              <div className="term-header">
                ...
                <span className="term-units">
                  Total Units: {utils.getTotalUnits(term)}
                </span>
              </div>
              <ScheduleDraggable
                courses={term.courses}
